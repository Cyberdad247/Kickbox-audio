...
function BrutalistMetricCard({
  ...
  return (
    ...
    <div className={`border border-white/10 bg-black/40 p-6 backdrop-blur-md transition-all hover:border-gold/40 hover:bg-black/60 ${className}`}>
      ...
      <div className="mt-6 flex items-end justify-between">
        {delta && (
          <span className={`text-[10px] uppercase font-bold tracking-wider ${positive ? 'text-emerald-400' : 'text-white/40'}`}>
            {delta}
          </span>
        )}
        {trend && <div className="h-8 w-24"><Sparkline data={trend} stroke={accent} /></div>}
      </div>
    </div>
  );
}
      ...
      <div className="grid grid-cols-12 gap-4">
          ...
          <div className="flex flex-col h-full justify-between">
            <div>
              ...
              <div className="mt-4 flex items-baseline gap-4">
                ...
                <span className="text-emerald-400 text-sm font-mono tracking-widest">+12.4% QTD</span>
              </div>
              <p className="mt-2 font-mono text-[10px] text-white/30 uppercase tracking-widest">{fullCurrency(valuation)} USD</p>
            </div>
        ...
        <div className="col-span-12 xl:col-span-4 border border-white/10 bg-black/60 p-1 backdrop-blur-md relative flex flex-col justify-between">
          <div className="absolute top-4 left-4 z-10">
             <div className="flex items-center gap-2">
                <span className={`h-2 w-2 rounded-full ${connected ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-amber-500'}`} />
          ...
          <div className="relative aspect-square sm:aspect-video xl:aspect-auto xl:h-full w-full overflow-hidden bg-[#050505] border border-white/5 group">
             {avatarActive ? (
                ...
                <div className="h-full w-full flex items-center justify-center relative">
                   ...
                   <div className="text-center z-10">
                      ...
                      <p className="font-mono text-[10px] text-white/40 uppercase tracking-widest">Sovereign Avatar Offline</p>
                   </div>
                </div>
             )}
             ...
             <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                <div className="flex flex-col gap-1">
                   {avatarActive && (
                      <span className={`text-[10px] font-mono tracking-widest uppercase ${speaking ? 'text-purple-400' : listening ? 'text-cyan-400' : 'text-white/40'}`}>
        ...
        <div className="col-span-12 border border-white/10 bg-black/40 p-8 backdrop-blur-md">
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-5 gap-0 border border-white/10 divide-y sm:divide-y-0 sm:divide-x divide-white/10 bg-black/20">
            {[
              { num: '1', title: 'Observe', data: 'VFS & 48kHz RMS', stat: 'Merkle 100% Valid', color: 'cyan' },
              { num: '2', title: 'Orient', data: 'MFOE Intent', stat: '99.7% Accuracy', color: 'purple' },
              { num: '3', title: 'Decide', data: 'Merlin_Ω DAG', stat: '4 Nodes Acyclic', color: 'gold' },
              { num: '4', title: 'Act', data: 'SmolVM Zone-0', stat: 'AST Tree-sitter', color: 'sky' },
              { num: '5', title: 'Verify', data: 'Z3 SAT Solver', stat: 'Firewall PASS', color: 'emerald' },
            ].map((stage, i) => (
              <div key={i} className="p-5 flex flex-col justify-between h-32 group hover:bg-white/5 transition-colors">
                <span className={`font-mono text-[9px] uppercase tracking-[0.2em] text-${stage.color}-400/60`}>
                  {stage.num}. {stage.title}
                </span>
                <div>
                  <span className={`block font-mono text-xs text-${stage.color}-400 mt-2`}>{stage.data}</span>
                  <span className="block text-[10px] text-white/40 font-mono tracking-widest mt-1">{stage.stat}</span>
                </div>
              </div>
            ))}
          </div>
