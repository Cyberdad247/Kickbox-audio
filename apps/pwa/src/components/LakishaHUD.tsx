...
export function LakishaHUD() {
  ...
  useWakeWord(() => {
    if (!isExpanded) {
      ...
      window.dispatchEvent(new CustomEvent('camelot:toggle-lakisha-hud', { detail: { expand: true } }));
    }
        ...
        <div className="flex flex-col gap-2 w-full px-2 animate-snap-in">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-mono text-[10px] uppercase text-white/80 group-hover:text-white">Lakisha</span>
            </div>
            <span className="text-[9px] uppercase tracking-widest text-[#00F0FF] opacity-0 group-hover:opacity-100 transition-opacity">Deploy</span>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1">
            <input
              ...
              placeholder={
                speaking ? 'Speaking…' : listening ? 'Listening…' : 'Speak or type voice command...'
              }
            ...
            {listening && (
              <div className="absolute right-3 top-1/2 flex -translate-y-1/2 items-center gap-1 z-10 pointer-events-none">
                <div className="w-1 bg-purple-400 rounded-full transition-all duration-[50ms]" style={{ height: `${Math.max(4, meter * 16)}px`, opacity: meter > 0.1 ? 1 : 0.4 }} />
                <div className="w-1 bg-purple-400 rounded-full transition-all duration-[75ms]" style={{ height: `${Math.max(6, meter * 24)}px`, opacity: meter > 0.1 ? 1 : 0.4 }} />
                <div className="w-1 bg-purple-400 rounded-full transition-all duration-[50ms]" style={{ height: `${Math.max(8, meter * 32)}px`, opacity: meter > 0.1 ? 1 : 0.4 }} />
                <div className="w-1 bg-[#00F0FF] rounded-full transition-all duration-[75ms]" style={{ height: `${Math.max(6, meter * 28)}px`, opacity: meter > 0.1 ? 1 : 0.4 }} />
                <div className="w-1 bg-[#00F0FF] rounded-full transition-all duration-[50ms]" style={{ height: `${Math.max(4, meter * 18)}px`, opacity: meter > 0.1 ? 1 : 0.4 }} />
              </div>
            )}
