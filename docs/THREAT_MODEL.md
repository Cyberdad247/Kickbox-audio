...
| **D**oS | Rate-limit flood on `/api/diagnostics/replay-coverage`               | **M**    | Upstash-Redis-backed sliding-window limiter (60 req / IP / 60 s) chained with a daily hard-circuit (1000 req / IP / 24 h) as a free-tier cost guardrail; 429 + `Retry-After: N` header. IP is `sha256`-hashed before storage (no raw PII in a third-party DB). Wired AFTER `ADMIN_TOKEN` auth (so unauth callers don't pollute the rate-limit state). In-memory fallback for dev/CI (fail-open + `console.warn` on Upstash outage). Multi-region accurate (v1.4.0). | low |
